...
let cont = "You come across a chamber that reaches upward to a shining light above. There is a long, winding staircase, and a much quicker, but rickety-looking ladder that leads up toward the light. Which do you take?";
let past = "The dragon wakes up and sits upright. You only have a moment to respond, to stay or run";
let stay = "You and the dragon have an uplifting conversation about local politics and become lifelong friends.";
...
if (Answer === 'left') {
    let Answer = window.prompt(left);
    if (Answer === 'follow') {
        let Answer = window.prompt(follow);
    } else if (Answer === 'continue') {
        let Answer = window.prompt(cont);
    } else (Answer === 'ladder'); {
        let Answer = window.prompt(ladder);
    } if (Answer === 'staircase') {
        let Answer = window.alert(staircase);
    }       

} else if (Answer === 'right') {
    let Answer = window.prompt(right);
    if (Answer === 'past') { 
    let Answer = window.prompt(past);
    } else (Answer === 'stay'); { 
     let Answer = window.prompt(stay);
} if (Answer === 'run') {
    let Answer = window.prompt(run);
}  else (Answer === 'away'); {
    let Answer = window.prompt(away);
} if (Answer === 'draw') {
    let Answer = window.prompt(draw); }
  else (Answer === 'pick'); {
        let Answer = window.alert;
    }
}
